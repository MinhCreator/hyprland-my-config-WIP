...
full_cleanup() {
    ...
    if sudo pacman -Rns $(pacman -Qdtq) &>/dev/null && yay -Rns $(yay -Qtdq) &>/dev/null; then
    ...
    echo -e "${YELLOW}5. Removing temporary files...${RESET}"
    if sudo yay -Scc && sudo pacman -Scc ; then
        echo -e "${GREEN}package cache removed successfully.${RESET}"
    else
        echo -e "${RED}Failed to remove package cache.${RESET}"
    fi


    # 11. Clean old config, dump file, empty directories and broken symlinks
    echo -e "${YELLOW}5. Removing temporary files...${RESET}"
    if rm -rf ~/.local/share/* && rmlint /home/$(whoami) && ./rmlint.sh; then
        echo -e "${GREEN}old config, dump file, empty directories and broken symlinks removed successfully.${RESET}"
    else
        echo -e "${RED}Failed to remove old config, dump file, empty directories and broken symlinks.${RESET}"
    fi
